...
| `mapImport.js` | Turning files the DM picks or drops into scenes: which kinds are accepted, how a whole selection is taken in one at a time, and where an animated map is written. Problems are collected and reported once at the end, so a long import never waits on a click. |
...
The whole suite also runs on every release, against the installed application rather than the
source - see "How a version reaches you" below.
