...
| `shapeBox.js` | The bounding box a picked shape wears, and the rotate and scale its eight handles drive. A corner or side handle scales against the opposite side; the ring just outside a corner rotates around the box centre. Shift holds proportion and snaps rotation to 15 degrees. A picked hole gets the same box, and stops where it would leave its room. |
