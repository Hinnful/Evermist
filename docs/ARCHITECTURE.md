...
| `musicPlan.js` | The music library's pure kernel: it reads a pasted YouTube link into a video or playlist id, reads a track's id and display name back out of its filename, filters the track list, and gives the equal-power level of a fade at a given phase. No DOM and no Electron, so it is unit-tested. |
| `music.js` | The music bubble at the top of the DM window: it lists whatever audio files sit in the app's music folder, plays one on a click and crossfades to the next, carries the volume and the filter, deletes a track, and holds the panel that reads a YouTube link and downloads the tracks picked from it. The folder is the whole library, so there is no database behind it. DM only. |
